...
func (repositorio Publicacoes) BuscarPorID(publicacoesID uint64) (modelos.Publicacao, error) {
	linha, erro := repositorio.db.Query("select p.*, u.nick from publicacoes p inner join usuarios u on u.id = p.autor_id where p.id = ?", publicacoesID)
	if erro != nil {
	...
	if linha.Next() {
		if erro = linha.Scan(&publicacao.ID, &publicacao.Titulo, &publicacao.Conteudo, &publicacao.AutorID, &publicacao.Curtidas, &publicacao.CriadoEm, &publicacao.AutorNick); erro != nil {
			return modelos.Publicacao{}, erro
		}
	}
...
func (repositorio Publicacoes) Buscar(ID uint64) ([]modelos.Publicacao, error) {
	...
	for linhas.Next() {
		var publicacao modelos.Publicacao
		if erro = linhas.Scan(&publicacao.ID, &publicacao.Titulo, &publicacao.Conteudo, &publicacao.AutorID, &publicacao.Curtidas, &publicacao.CriadoEm, &publicacao.AutorNick); erro != nil {
			return nil, erro
		}
